...
- [\<Measure of Mode of Writing down important point, and take quizzes
  and uses
  results\>](#measure-of-mode-of-writing-down-important-point-and-take-quizzes-and-uses-results)
...
of **“storytelling using the data.”** The goal is to analyse the Student
dataset and try to find a relation between the various variables and the
exam mark( which variables or combination or variables affect the exam
marks the most).*</span>
...
First we checked to see if the variable writing down important points,
and take quizzes and use results were numeric. Next we measured the
freqency of the outputs for this variable.

We found that a majority of the Students specifically 37.62% frequency
of 4 and 37.62% frequency of 5 wrote down important notes and took
quizzes and used results.
...
cbind(frequency = table(Student_dataset_write_down_important_points_freq), percentage = prop.table(table(Student_dataset_write_down_important_points_freq)) *
    100)  #For getting frequency of writing down important points.
```
...
cbind(frequency = table(Student_dataset_take_quizzes_and_use_results_freq), percentage = prop.table(table(Student_dataset_take_quizzes_and_use_results_freq)) *
    100)  #For getting frequency of take quizzes and use results.
```
...
The Covariance of Write down important points is and that of Take
quizzes and use results is 0.4099010

## \<Measure the skewness of each variable\>

We proceeded to measure the skewness of the variables we chose to use
for this dataset namely Write down important points, Reorganise course
outline and Take quizzes and use results. Take quizzes and use results
as well as Write down important points is negatively (left skew),
whereas Recognise course outline is Symmetrical.
